function ZZ = NewPowerSpectrum(Filename)
...
nfft=1024;
cols = ceil((length(sig)-30)/20);% 20 30 待定
window=hamming(cols);%窗 98个点  30ms
% window1=hamming(30); %海明窗
% window2=blackman(30); %blackman窗
overlap=66; %数据重叠  重叠了20ms
range='half';
...
% 为了便于fft批量化处理 这里将alpha(a>0.5的部分)设置为0.7
alpha=0.7;
Yf=[];
Yf(1)=alpha*Y(1);
for i = 2:len
...
% 为了便于fft批量化处理 这里将alpha(a<0.5的部分)设置为0.2
alpha=0.2;
Ys=[];
Ys(1)=alpha*Y(1);
for i = 2:len
